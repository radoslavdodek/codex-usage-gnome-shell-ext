...
export function subprocessPathForCommand(command, basePath = GLib.getenv('PATH') ?? '') {
    if (typeof command !== 'string' || !command.includes('/'))
        return basePath;

    const directory = GLib.path_get_dirname(command);
    if (!directory || directory === '.')
        return basePath;

    const entries = String(basePath).split(':').filter(Boolean);
...
function spawnSubprocess(argv, flags) {
    const launcher = new Gio.SubprocessLauncher({flags});
    launcher.setenv('PATH', subprocessPathForCommand(argv[0]), true);
...
export function normalizeCodexRateLimitsPayload(result, options = {}) {
    ...
    const snapshot = createSnapshot({
        ...
        fiveHour,
        weekly,
        overallStatus: hasBucketError ? OVERALL_STATUSES.ERROR : null,
        generatedAtUnix: now,
...
function subprocessCommunicate(argv, input, timeoutSeconds, cancellable) {
    return new Promise((resolve, reject) => {
        let subprocess;
        try {
            subprocess = spawnSubprocess(argv, Gio.SubprocessFlags.STDIN_PIPE | Gio.SubprocessFlags.STDOUT_PIPE | Gio.SubprocessFlags.STDERR_PIPE);
        } catch (error) {
...
function appServerReadRateLimits(command, timeoutSeconds, cancellable) {
    return new Promise((resolve, reject) => {
        let subprocess;
        try {
            subprocess = spawnSubprocess([command, 'app-server', '--listen', 'stdio://'], Gio.SubprocessFlags.STDIN_PIPE | Gio.SubprocessFlags.STDOUT_PIPE | Gio.SubprocessFlags.STDERR_PIPE);
        } catch (error) {
            reject(new ProviderError(REFRESH_STATES.NOT_CONFIGURED, sanitizeError(error, 'Unable to start Codex command.'), {cause: error}));
            return;
        }
