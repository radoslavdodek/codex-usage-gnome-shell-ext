...
- [X] T001 Create root extension skeleton files in `extension.js`, `metadata.json`, `prefs.js`, and `stylesheet.css`
- [X] T002 Create runtime module skeleton files in `lib/balanceSource.js`, `lib/codexAppServerSource.js`, `lib/mockSource.js`, `lib/model.js`, `lib/formatter.js`, `lib/redaction.js`, `lib/settings.js`, and `lib/compatibility.js`
- [X] T003 [P] Create GSettings schema skeleton in `schemas/org.gnome.shell.extensions.codex-usage.gschema.xml`
- [X] T004 [P] Create non-Shell test harness entry point in `tests/run-tests.sh`
- [X] T005 [P] Create initial fixture directory and fixture README in `tests/fixtures/README.md`
...
- [X] T006 Define extension metadata, UUID, supported GNOME Shell versions 46, 47, 48, and 49, and stylesheet/schema references in `metadata.json`
- [X] T007 Define preferences keys, defaults, ranges, and enum values in `schemas/org.gnome.shell.extensions.codex-usage.gschema.xml`
- [X] T008 [P] Implement `CodexBalanceSnapshot`, `BalanceBucket`, `SourceAuthContext`, `DataSourceConfig`, and `RefreshState` validation helpers in `lib/model.js`
- [X] T009 [P] Implement the `BalanceSource` interface contract and shared provider error types in `lib/balanceSource.js`
- [X] T010 [P] Implement centralized sensitive-value redaction helpers in `lib/redaction.js`
- [X] T011 [P] Implement status, percent, bucket-priority, and local-time formatting helpers in `lib/formatter.js`
- [X] T012 [P] Implement settings load, validation, and fallback helpers in `lib/settings.js`
- [X] T013 [P] Implement GNOME Shell 46-49 compatibility helper functions in `lib/compatibility.js`
- [X] T014 Implement enable/disable lifecycle scaffolding with no UI, timers, settings bindings, source work, or external access before `enable()` in `extension.js`
- [X] T015 [P] Implement preferences UI scaffolding for refresh interval, warning threshold, display format, bucket priority, source kind, and Codex command path in `prefs.js`
...
- [X] T016 [P] [US1] Add formatter tests for lowest-bucket display text, warning threshold, limit reached, and display format variants in `tests/formatter.test.js`
- [X] T017 [P] [US1] Add snapshot/status mapping tests for normal, warning, limit reached, loading, partial bucket, and stale-safe panel states in `tests/balanceSource.test.js`

### Implementation for User Story 1

- [X] T018 [P] [US1] Implement deterministic mock snapshots for loading, normal, warning, limit reached, stale, unavailable, and error states in `lib/mockSource.js`
- [X] T019 [US1] Implement compact panel display text derivation for `bucket-percent`, `percent-only`, and `state-label` formats in `lib/formatter.js`
- [X] T020 [US1] Wire extension controller state to settings and the selected source without starting duplicate providers in `extension.js`
- [X] T021 [US1] Render exactly one owned top-bar indicator with compact label and status style classes in `extension.js`
- [X] T022 [US1] Add panel indicator style classes for loading, normal, warning, limit reached, stale, not-authenticated, not-configured, and error states in `stylesheet.css`
- [X] T023 [US1] Record manual enable/display verification steps and expected outcomes for the compact indicator in `specs/001-codex-balance-indicator/quickstart.md`
...
- [X] T024 [P] [US2] Add formatter tests for 5-hour and weekly bucket row labels, local timezone reset formatting, reset-text fallback, and unavailable bucket text in `tests/formatter.test.js`
- [X] T025 [P] [US2] Add menu state fixture data for successful, partial-bucket, stale, and error snapshots in `tests/fixtures/menu-states.json`

### Implementation for User Story 2

- [X] T026 [US2] Build dropdown menu rows for `fiveHour` and `weekly` bucket labels, percentages, reset values, and bucket statuses in `extension.js`
- [X] T027 [US2] Render last successful refresh time, current overall data state, sanitized message row, and manual refresh action in `extension.js`
- [X] T028 [US2] Implement bucket reset-time and fallback reset-text formatting for menu rows in `lib/formatter.js`
- [X] T029 [US2] Add dropdown row, status, and action styles in `stylesheet.css`
- [X] T030 [US2] Preserve visible valid bucket rows when the other bucket is unavailable or errored in `extension.js`
...
- [X] T031 [P] [US3] Add Codex app-server success, partial-window, malformed, API-key-auth, unauthenticated, timeout, and rate-limited fixtures in `tests/fixtures/codex-app-server.json`
- [X] T032 [P] [US3] Add Codex app-server parsing tests for `rateLimitsByLimitId.codex`, `limitId: "codex"` fallback, 300-minute primary mapping, 10080-minute secondary mapping, and `100 - usedPercent` normalization in `tests/balanceSource.test.js`
- [X] T033 [P] [US3] Add stale-after, no-overlap, cancellation, timeout, and failed-with-previous refresh tests in `tests/balanceSource.test.js`

### Implementation for User Story 3

- [X] T034 [US3] Implement async `codex app-server --listen stdio://` subprocess setup, JSON-RPC initialize, and `account/rateLimits/read` request flow in `lib/codexAppServerSource.js`
- [X] T035 [US3] Implement ChatGPT auth-mode preflight and explicit API-key auth rejection without reading or falling back to `OPENAI_API_KEY` in `lib/codexAppServerSource.js`
- [X] T036 [US3] Normalize Codex app-server primary and secondary windows into `fiveHour` and `weekly` buckets in `lib/codexAppServerSource.js`
...
- [X] T041 [P] [US4] Expand redaction tests for authorization headers, cookies, bearer tokens, token-like key/value pairs, account identifiers, high-entropy strings, and raw payload fragments in `tests/redaction.test.js`
- [X] T042 [P] [US4] Add source failure mapping tests for not-authenticated, not-configured, wrong-auth-mode, malformed, rate-limited, timeout, and generic error snapshots in `tests/balanceSource.test.js`

### Implementation for User Story 4

- [X] T043 [US4] Route provider stdout, stderr, JSON-RPC errors, and thrown errors through redaction before UI or logging in `lib/codexAppServerSource.js` and `lib/redaction.js`
- [X] T044 [US4] Map provider failure kinds to normalized overall statuses, bucket statuses, and sanitized detail messages in `lib/model.js` and `lib/formatter.js`
...
- [X] T050 [US5] Audit and complete cleanup of panel item, menu rows, timers, settings bindings, cancellables, source instances, and signal handlers in `extension.js`
- [X] T051 [US5] Apply GNOME Shell 46-49 compatibility helper usage for panel, menu, settings, and preferences APIs in `lib/compatibility.js`, `extension.js`, and `prefs.js`
- [X] T052 [P] [US5] Document package contents, local install, schema compilation, enable/disable, and real-source setup in `README.md`
- [X] T053 [US5] Verify package scope excludes credentials, tokens, raw captured payloads, generated test artifacts, and unrelated project files in `metadata.json` and `README.md`
